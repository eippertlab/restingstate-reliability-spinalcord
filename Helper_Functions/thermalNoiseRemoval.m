function thermalNoiseRemoval(sessionName, rawData)
...
% Set up file naming
dataInput   = rawData;
dataOutput  = [sessionName '_denoised.nii.gz'];
dataNoise   = [sessionName '_noise.nii.gz'];
dataNoiseA  = [sessionName '_noiseAfter.nii.gz'];
dataNumComp = [sessionName '_numSignalComp.nii.gz'];
dataResid   = [sessionName '_residuals.nii.gz'];
...
unix(['fslmaths ' inData ' -Tmean $(remove_ext ' inData ')_mean.nii.gz']);
unix(['fslmaths ' inData ' -Tstd $(remove_ext ' inData ')_sd.nii.gz']);
unix(['fslmaths $(remove_ext ' inData ')_mean.nii.gz -div $(remove_ext ' inData ')_sd.nii.gz $(remove_ext ' inData ')_tsnr.nii.gz']);